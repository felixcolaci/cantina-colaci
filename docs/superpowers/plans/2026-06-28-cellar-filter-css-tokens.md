# Keller-Filterleiste CSS-Tokens Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace all Tailwind color/appearance classes in the cellar filter bar with inline CSS custom property styles, matching the design system used by BottomNav.

**Architecture:** Single-file change in `app/cellar/page.tsx`. Filter pills switch from `cn(...)` conditional Tailwind classes to inline style objects with `var(--...)` tokens. The amber limit banner gets design-system token styling. The `cn` import is removed as it becomes unused.
...
- Layout/structural Tailwind utilities (`flex`, `fixed`, `overflow-x-auto`, etc.) are acceptable — only appearance classes are replaced
- Font body: `var(--font-body)` (Plus Jakarta Sans)
- Run build: `npm run build`

---

### Task 1: Replace Tailwind appearance classes with CSS tokens

**Files:**
- Modify: `app/cellar/page.tsx`

**Interfaces:**
- Consumes: CSS tokens from `app/globals.css` — `--primary`, `--primary-foreground`, `--background`, `--foreground`, `--border`, `--warning`, `--radius-full`, `--radius-md`, `--space-1`, `--space-3`, `--space-4`, `--text-sm`, `--text-xl`, `--font-body`, `--duration-fast`, `--ease-standard`

- [ ] **Step 1: Replace the full file**

Write the complete updated `app/cellar/page.tsx` — all filter pill classNames replaced with inline styles, limit banner updated, `cn` import removed:
...
git commit -m "style: replace Tailwind color classes with CSS tokens in cellar filter bar"
```
